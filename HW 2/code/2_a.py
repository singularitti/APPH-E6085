...
def density(rr, alpha):
    vec_overlap_integrand = np.vectorize(
        lambda rr, a, b: np.exp(-(a + b) * rr ** 2))  # This is not equal to overlap_integrand
...
def plot():
    ...
    fig2 = plt.figure()
    plt.plot(r, [distribution_probability(rr, alpha_1)
                 for rr in r], label=r'$n=1$')
    plt.plot(r, [distribution_probability(rr, alpha_2)
                 for rr in r], label=r'$n=2$')
    plt.plot(r, [distribution_probability(rr, alpha_3)
                 for rr in r], label=r'$n=3$')
    plt.plot(r, [distribution_probability(rr, alpha_4)
                 for rr in r], label=r'$n=4$')
    plt.plot(r, [exact_distribution_probability(rr)
                 for rr in r], label=r'exact value')
    plt.xlabel(r'$r$ (Bohr)', fontsize=16)
