...
class EigenvalueProblem(object):
    ...
    def hartree_integrand(self, coeff):
        def hartree_potential(rr2):
            ...
            return np.array(
                [
                    coeff[ii]
                    * coeff[jj]
                    * np.exp(-(self.alpha[ii] + self.alpha[jj]) * rr2**2)
                    for ii in range(0, 4)
                    for jj in range(0, 4)
                ]
            ).sum()
        ...
        def length(theta, rr1, rr2):
            return 1 / np.sqrt(rr1**2 + rr2**2 - 2 * rr1 * rr2 * np.cos(theta))
        ...
        def tmp(theta, rr1, rr2):
            return (
                ...
                * rr2**2
                * np.sin(theta)
                * hartree_potential(rr2)
                * length(theta, rr1, rr2)
            )

        def integrand(ii, jj, theta, rr1, rr2):
            return np.exp(-(self.alpha[ii] + self.alpha[jj]) * rr1**2) * tmp(
                theta, rr1, rr2
            )

        return [
            nquad(
                lambda theta, rr1, rr2: integrand(i, j, theta, rr1, rr2),
                [[0, np.pi], [0, np.inf], [0, np.inf]],
            )
            for i in range(0, 4)
            for j in range(0, 4)
        ]
